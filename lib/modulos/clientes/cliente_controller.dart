...
class ClienteController extends ControllerBase {
  ...
  Future<Response> listar(RequestContext context) {
    return executar(() async {
      final dados = validar(ClienteSchema.listar, queryParams(context));
      final usuarioId = context.read<ContextoUsuario>().id;
      final clientes = await _service.listar(
        usuarioId: usuarioId,
        organizacaoId: dados.organizacaoId,
      );
      return RespostaJsonUtil.sucessoLista(
        dados: clientes.map((e) => e.toMap()).toList(),
      );
    });
  }
  ...
  Future<Response> cadastrar(RequestContext context) {
    return executar(() async {
      ...
      final dados = validar(ClienteSchema.cadastrar, corpo);
      final usuarioId = context.read<ContextoUsuario>().id;
      final cliente = await _service.cadastrar(
        usuarioId: usuarioId,
        entrada: dados,
      );
  ...
  Future<Response> atualizar(RequestContext context, String id) {
    return executar(() async {
      final corpo = await context.request.json();
      final dados = validar(ClienteSchema.atualizar, corpo);
      final usuarioId = context.read<ContextoUsuario>().id;
      final cliente = await _service.atualizar(
        usuarioId: usuarioId,
        id: id,
        entrada: dados,
      );
      return RespostaJsonUtil.sucesso(dados: cliente.toMap());
    });
  }
